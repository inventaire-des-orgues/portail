            ...
            with open(options['codestable'][0], "r", encoding="utf-8") as f:
                ...
                for couple_facteur in couples_facteurs:
                    ...
                    if facteur_delete.count() == 0:
                        print("Le nom du facteur à remplacer n'existe pas : {}".format(facteur_avant))
                    elif facteur_delete.count() > 1:
                        print("Deux facteurs à effacer portent le même nom : {}".format(facteur_avant))
                    else:
                        ...
                        for f in range(1, len(couple_facteur)):
                            ...
                            if manufacture_replace.count() == 0:
                                erreur = True
                                print("Le nom de la manufacture qui doit prendre place n'existe pas : {}".format(manufacture_apres))
                            elif manufacture_replace.count() > 1:
                                erreur = True
                                print("Deux manufactures à mettre portent le même nom : {}".format(manufacture_apres))
                        if not erreur:
                            print("Je cherche à remplacer {} par {}".format(facteur_delete, liste_manufacture_apres))
                            ...
                            for orgue in orgues:
                                ...
                                orgue.save()
                            if options['delete']:
                                print("Le facteur {} a été retiré de la liste.".format(facteur_delete))
